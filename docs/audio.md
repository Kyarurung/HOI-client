...
데프콘 이미지와 그 아래 % 표시는 간격을 제외한 두 개의 독립 클릭 영역이며 같은 세계 긴장도 창을 연다. 이 두 버튼과 세계 긴장도 창의 날짜·국가·세계 긴장도·현재 전쟁·닫기 버튼은 모두 `hoi:ui.click`을 사용한다. 원본 TFR `interface/ingamemenu.gui`의 `menu_gamesettings_button`에 지정된 `click_default`와 같은 음원이며, 외부 팩의 `sound/menu/click_default.wav` 변환본을 그대로 재생한다. 해당 화면의 버튼은 Minecraft 기본 버튼음 대신 이 효과음을 한 번 재생한다. 다른 화면의 전용 닫기 효과음은 유지한다.

세계 긴장도 이력의 닫기 글자는 실제 글자 배율과 버튼 높이에 맞춰 1~3 GUI 위로 보정한다. 일반적인 0.5 패널 배율에서는 2 GUI이며 버튼 위치·클릭 영역은 유지한다.

`/hoi stop`의 STOP 신호는 음악뿐 아니라 열린 이벤트 창과 표시 대기 중인 이벤트도 즉시 폐기한다. 접속 종료도 같은 팝업 정리 경로를 사용한다. 클라이언트 GameTest에서 열린 슈퍼 이벤트와 ESC 메뉴 뒤에서 대기하는 슈퍼 이벤트를 각각 종료하고 다음 틱에 재표시되지 않는지 검사한다.
