...
All UI images come from the private HOI-resourcepack repository. Build its `scripts/build_pack.py` first; rendering tests use `../HOI-resourcepack/build/resources.zip` or `-PhoiResourcePack=<absolute path>`. Never embed images or map models in this JAR. Keep category and tabs art identical, research tabs icon-only, and preserve image aspect ratios with bounded drawing areas. Sidebar widths are 30%, trade/economy 35%, politics/recruitment 40%. Use at most six visible research slots per page and allocate spare vertical space to the top banner using aspect-preserving crop-to-fill; preserve additional server slots through pagination. Agency mutations use server-issued choices and session revisions; stale responses cannot reopen or preserve a revoked private screen.
