...
/** Metal rails and recessed surfaces, drawn without bundling any image assets. */
final class HoiMenuStyle {
    static final int TEXT = 0xFFE3E1D8, ACCENT = 0xFFB8BDB6, MUTED = 0xFF9B9D97;
    private HoiMenuStyle() {}

    static void bevel(GuiGraphicsExtractor g, int x, int y, int w, int h, boolean inset) {
    ...
    static void metal(GuiGraphicsExtractor g, int x, int y, int w, int h) {
    ...
    static void recess(GuiGraphicsExtractor g, int x, int y, int w, int h) {
        g.fillGradient(x, y, x + w, y + h, 0xFF0B0C0E, 0xFF191A1F);
        bevel(g, x, y, w, h, true);
    }

    static void control(GuiGraphicsExtractor g, int x, int y, int w, int h, boolean selected, boolean hover) {
        if (selected) {
