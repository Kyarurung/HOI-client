...
public class DialogScreen extends Screen {
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g, int mx, int my, float delta) {
        ...
        text(g, Component.literal(trim(view.title(), (int)((left + pane - 14 - start) / textScale()))).getVisualOrderText(), start, top + 35, ink);
        text(g, Component.literal(trim(view.subtitle(), (int)((pane - 28) / textScale()))).getVisualOrderText(), left + 12, top + 48, paper() ? 0xff62566a : HoiMenuStyle.MUTED);
        ...
        if (view.kind() != DialogView.Kind.STATE) {
            for (var line : font.split(Component.literal(view.body()), (int)((pane - 32) / textScale()))) { text(g, line, left + 15, y, ink); y += paper() ? 12 : 13; }
        ...
        if (view.kind() == DialogView.Kind.STATE) {
            ...
            text(g, Component.literal(trim(tile.name() + (tile.value().isEmpty() ? "" : " · " + tile.value()), (int)((pane - 32) / textScale()))).getVisualOrderText(), left + 15, y, ink); y += paper() ? 15 : 17;
            for (var line : font.split(Component.literal(tile.detail()), (int)((pane - 40) / textScale()))) { text(g, line, left + 19, y, ink); y += paper() ? 12 : 13; }
            y += 15;
        }
    ...
    private void text(GuiGraphicsExtractor g, net.minecraft.util.FormattedCharSequence text, int x, int y, int color) {
        g.pose().pushMatrix(); g.pose().translate(x, y); g.pose().scale(textScale());
