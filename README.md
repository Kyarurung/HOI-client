...
연구 슬롯 배치는 최대 6개까지 수용하며 실제로는 서버가 제공한 슬롯만 표시합니다. 현재 국가의 최대 슬롯 수는 5개이고 클라이언트에서 빈 슬롯을 추가하지 않습니다. 페이지 번호와 좌우 이동 버튼은 표시하지 않습니다. 슬롯 원본 이미지는 배경으로 유지하고 진행 중인 기술 이미지를 슬롯 중앙에 겹쳐 표시합니다. 연구 선택 라벨에는 안쪽 여백을 주며 저장 일수 문구는 슬롯에서 숨기되 서버의 저장 일수와 남은 연구 기간 계산은 유지합니다. 남는 세로 공간은 상단 연구 배너에 배분합니다. 내부 8개 연구 분야를 유지하며 화면에는 보병, 지상 & 항공 지원, 기갑, 포, 해군, 해군 지원 장비, 공군, 공학, 산업의 9개 아이콘 탭을 표시합니다. 탭에는 라벨을 붙이지 않고 툴팁과 접근성 이름만 제공합니다. `category/`와 `tabs/`는 같은 원작 UI 아이콘을 사용하며, 선택한 탭의 아래 테두리는 연구 패널에 이어집니다. 기술 상세에도 해당 아이콘 탭을 부착합니다. 모든 이미지는 원본 비율을 유지합니다. 상단 연구 배너만 영역을 채우도록 중앙 부분을 잘라 표시하고 나머지는 지정 영역 안에 중앙 정렬하며, 탭·사진의 최대 크기는 패널 너비와 독립적으로 제한합니다.
